...
<body onLoad="init(<?php echo htmlspecialchars($_GET["zeitraum"]) . ',' . htmlspecialchars($_GET["raumid"]) . ',\'' . htmlspecialchars($_GET["raumNr"]) . '\',' . htmlspecialchars($_GET["tag"]) . ',' . htmlspecialchars($_GET["monat"]) . ',' . htmlspecialchars($_GET["jahr"]) . ',' . htmlspecialchars($_GET["mitSamstag"]) . ',' . htmlspecialchars($_GET["mitSonntag"]) ?>)"><!--rmrFrageStandorteAb() -->
	<div id="header">
			<div id="hread">
	...
	<span class="info1">
		<p>
			...
			<b>Heatmap:</b></br> Zeigt die prozentuale Belegung eines Raums in einem ausgewählten Zeitraum zu verschiedenen Uhrzeiten an.</br></br>
					...
					<input type="button" onClick="init()" value="anzeigen"/>
...
<!--	<?php echo htmlspecialchars($_GET["zeitraum"]) . ',' . htmlspecialchars($_GET["raumid"]) . ',' . htmlspecialchars($_GET["raumNr"]) . ',' . htmlspecialchars($_GET["tag"]) . ',' . htmlspecialchars($_GET["monat"]) . ',' . htmlspecialchars($_GET["jahr"]) . ',' . htmlspecialchars($_GET["mitSamstag"]) . ',' . htmlspecialchars($_GET["mitSonntag"]) ?> -->
</body>
</html> 